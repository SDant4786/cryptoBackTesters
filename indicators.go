...
func calculateCloud(prices []float64) *Cloud {
	cloud := &Cloud{}

	// Calculate the high and low prices over the past 9 periods.
	highs := prices[len(prices)-9:]
	lows := prices[len(prices)-9:]
	for _, price := range prices[:len(prices)-9] {
		if price > highs[0] {
			highs[0] = price
		}
		if price < lows[0] {
	...
	cloud.TenkanSen = (highs[0] + lows[0]) / 2

	// Calculate the Kijun-sen (base line).
	sum := 0.0
	for _, price := range prices[len(prices)-26:] {
		sum += price
	}
	cloud.KijunSen = sum / 26

	// Calculate the Senkou Span A (leading span A).
	cloud.SenkouSpanA = (cloud.TenkanSen + cloud.KijunSen) / 2

	// Calculate the highest high and lowest low over the past 52 periods.
	high := highs[0]
	low := lows[0]
	for _, price := range prices[:len(prices)-52] {
		if price > high {
			high = price
		}
		if price < low {
	...
	cloud.SenkouSpanB = (high + low) / 2

	// Calculate the Chikou Span (lagging span).
	cloud.ChikouSpan = prices[0]

	// Calculate the Conversion Line and Base Line.
	cloud.ConversionLine = (cloud.TenkanSen + cloud.KijunSen) / 2
	cloud.BaseLine = (cloud.SenkouSpanA + cloud.SenkouSpanB) / 2
...
func HMA(candles []coinbasepro.HistoricRate, period float64, weight float64) []float64 {
	var hmas []float64
...
func WMA(candles []coinbasepro.HistoricRate, period float64) []float64 {
	var wmas []float64
	totDen := 0.0
	for i := period; i > 0; i-- {
		totDen += i
	}
	for i := 0; i < len(candles)-int(period); i++ {
		total := 0.0
		n := 1.0
		for j := i; j < int(period)+i; j++ {
			total += candles[j].Close * (n / totDen)
			n++
		}
		wmas = append(wmas, total)
	}
	return wmas
}
func VWWMA(candles []coinbasepro.HistoricRate, period float64) []float64 {
	var vwwmas []float64
	totWDen := 0.0
	for i := period; i > 0; i-- {
		totWDen += i
	...
	for i := 0; i < len(candles)-int(period); i++ {
		...
		n := 1.0
		for j := i; j < int(period)+i; j++ {
			num += candles[j].Close * candles[j].Volume * (n / totWDen)
			den += candles[j].Volume * (n / totWDen)
			n++
		}

		vwwmas = append(vwwmas, num/den)
	}
	return vwwmas
}
